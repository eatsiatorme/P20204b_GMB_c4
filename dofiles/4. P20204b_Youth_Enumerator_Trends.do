copy "$data_quality\Output templates\Enumerator_Trends.xlsx"  "$data_quality\05_output\Enumerator_Trends.xlsx", replace

cd "$dofiles"
include "4.1. ${proj}_${tool}_Mata.do"
...
collapse (sum)status_1 duration_m, by(interview_date ${enumerator_id})
...
foreach var of varlist $outcome {
	distinct ${enumerator_id} if `var'!=.
	if `r(ndistinct)' < 2 {
	di "Less than two groups - can't do test"
	global outcome : list outcome-`var'
	}
}


inttrend duration_m $trigger $outcome using `interviewers', interviewer(${enumerator_id}) 
...
local rowbeg = `r(N)' + 1
di "`rowbeg'"
export excel "$data_quality\05_output\Enumerator_Trends.xlsx", sheet("Contact Time", modify) keepcellfmt cell(A2)
mata: check_list_format("$data_quality\05_output\Enumerator_Trends.xlsx", "Contact Time", "${enumerator_id}", 1, 2, `rowbeg', `n_vars')	
}
