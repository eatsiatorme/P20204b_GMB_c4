...
local files : dir "$exported\media" file "Comments*.csv", respectcase	
...
duplicates drop comment variable `fname', force
...
keep submissiondate $unique_id $supervisor_id variable label value comment message $enumerator_id
order  submissiondate $unique_id $supervisor_id variable label value comment message $enumerator_id

gen x1 = $supervisor_id
gen x2 = $enumerator_id
drop $supervisor_id $enumerator_id
rename (x1 x2) ($supervisor_id $enumerator_id)
